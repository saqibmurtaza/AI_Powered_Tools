...
export default function TenureCalculatorPage() {
  ...
  // AI Function using Hugging Face Inference API
  const generateAISummary = async (years: number, months: number) => {
    try {
      const response = await fetch(
        "https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.2",
        {
          ...
          method: "POST",
          body: JSON.stringify({
            inputs: `<s>[INST] Write a professional one-sentence resume summary for a worker with ${years} years and ${months} months of experience. Focus on their professional stability. [/INST]`,
          }),
        }
      );
      ...
      return text.includes("[/INST]") ? text.split('[/INST]')[1].trim() : text;
    } catch (error) {
      console.error("AI Error:", error);
      return "Calculated tenure successfully. Highlight this consistency in your professional profile.";
    }
  };

  const calculateTenure = async () => {
    if (!startDate || !endDate) return;
    ...
    // Trigger AI Insight
    setIsAiLoading(true);
    const insight = await generateAISummary(years, months);
    setAiInsight(insight);
    setIsAiLoading(false);
  };

  const features = [
    { icon: <Briefcase className="w-6 h-6" />, title: "Employment Verification", description: "Accurately calculate years of service for HR audits, pension eligibility, and resume building." },
    { icon: <Clock className="w-6 h-6" />, title: "Project Durations", description: "Measure the exact length of long-term projects or contract engagements with precision." },
    { icon: <TrendingUp className="w-6 h-6" />, title: "Seniority Tracking", description: "Determine organizational seniority for promotions, benefits, and internal restructuring." },
    { icon: <ShieldCheck className="w-6 h-6" />, title: "Legal Compliance", description: "Ensure service period accuracy for severance calculations and labor law requirements." }
  ];

  const applications = [
    { title: "Human Resources", items: ["Service award eligibility", "Leave balance accrual", "Onboarding/Offboarding stats"] },
    { title: "Project Management", items: ["Historical timeline analysis", "Vendor contract duration", "Resource allocation planning"] },
    { title: "Legal & Finance", items: ["Severance pay estimation", "Vesting period tracking", "Tax residency durations"] }
  ];
    ...
    <div className="min-h-screen bg-white font-poppins">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          <div className="lg:col-span-1">
            <div className="bg-gray-50 rounded-2xl p-8 border h-full">
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Start Date (Joining Date)</label>
                  <input
                    type="date"
                    value={startDate}
                    onChange={(e) => setStartDate(e.target.value)}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">End Date (or Today)</label>
                  <input
                    type="date"
                    value={endDate}
                    onChange={(e) => setEndDate(e.target.value)}
                ...
                <button
                  onClick={calculateTenure}
                  className="w-full bg-gradient-to-r from-[#4D0682] to-[#7C3AED] text-white py-3 px-4 rounded-lg font-semibold hover:opacity-90 shadow-lg transition-all"
                >
                  Calculate Tenure
                </button>
                ...
                {tenure && (
                  <div className="mt-6 space-y-4">
                    <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm text-center">
                      <h3 className="text-lg font-semibold mb-4 text-gray-900 border-b pb-2">Tenure Result</h3>
                      <div className="space-y-2">
                        <div className="text-3xl font-bold text-gray-900">{tenure.years} <span className="text-sm font-normal text-gray-600">years</span></div>
                        <div className="text-xl text-gray-700">{tenure.months} <span className="text-sm text-gray-600">months</span></div>
                    ...
                    {/* AI Powered Section */}
                    <div className="p-5 bg-gradient-to-br from-purple-50 to-white rounded-xl border border-purple-100 shadow-sm relative overflow-hidden">
                      <div className="flex items-center gap-2 mb-2 text-[#4D0682] font-bold text-xs uppercase tracking-tighter">
        ...
        <div className="bg-gray-50 rounded-2xl p-8 border mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {applications.map((app, index) => (
              <div key={index} className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm transition-all hover:-translate-y-1">
                <h3 className="font-semibold text-gray-900 mb-4 text-lg flex items-center">
                  <div className="w-2 h-2 bg-gradient-to-r from-[#4D0682] to-[#7C3AED] rounded-full mr-3"></div>
                  {app.title}
                </h3>
                <ul className="space-y-2">
                  {app.items.map((item, i) => (
                    <li key={i} className="flex items-start text-sm text-gray-600">
                      <span className="text-[#7C3AED] mr-2">•</span>{item}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="bg-gradient-to-r from-[#4D0682] to-[#7C3AED] rounded-2xl p-8 text-white text-center">
          <h2 className="text-2xl font-bold mb-4">AI-Powered Professional Analysis</h2>
          <p className="text-white/80 text-sm max-w-2xl mx-auto">
            Leveraging machine learning to help you understand your professional journey. Get precise service data for your documentation and resume.
          </p>
        </div>
      </main>
      <Footer />
