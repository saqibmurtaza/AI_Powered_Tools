...
export default function ClientPage(): JSX.Element {
  ...
  const [summaries, setSummaries] = useState<Record<string, CardSummaryState>>({});
  ...
  async function summarizeCard(cardId: string, text: string) {
    try {
      ...
      if (!res.ok) {
        ...
        setSummaries((s) => ({ ...s, [cardId]: { content: "AI failed to generate a summary.", isFallback: true } }));
        return;
      }
      const data = await res.json();
      const summary = data?.summary || "No summary returned.";
      setSummaries((s) => ({ ...s, [cardId]: { content: summary, isFallback: Boolean(data?.fallback) } }));
    } catch (err) {
      console.error("summarizeCard error", err);
      setSummaries((s) => ({ ...s, [cardId]: { content: "Error generating summary.", isFallback: true } }));
    } finally {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
          ...
          <div className="text-center mb-8 sm:mb-12">
            ...
            <p className="text-lg sm:text-xl text-gray-600 max-w-2xl mx-auto px-4">
              Keep your thoughts, research, and ideas organized using customizable tags and categories.
            </p>
          </div>
                      ...
                      <CardContent className="p-4 sm:p-6">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                          <div className="flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-purple-700">
                              {summary.isFallback
                                ? "Quick local summary shown because the AI service is unavailable right now."
                                : "Generated using AI."}
                            </p>
                          </div>
                  ...
                  {summaries[c.id] && (
                    <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg border border-purple-100">
                      <div className="text-xs font-semibold text-purple-700 mb-1">
                        {summaries[c.id].isFallback ? "Quick Summary" : "AI Summary"}
                      </div>
                      <div className="text-xs text-purple-600 mb-2">
                        {summaries[c.id].isFallback
                          ? "AI service is unavailable right now, so this is a basic local summary."
                          : "Generated using AI."}
                      </div>
                      <div className="text-sm text-gray-700 whitespace-pre-line">{summaries[c.id].content}</div>
                    </div>
                  )}
                </motion.article>
              ))}
